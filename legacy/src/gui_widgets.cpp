...
void widgets::TankView::DrawStatic(ScreenRenderSurface & screen)
{
    ...
    for (int y = 0; y < this->screen_rect.size.y; y++) {
        for (int x = 0; x < this->screen_rect.size.x; x++)
        {
            ...
            if (!tank_energy)
            {
                screen.SetPixel({x + this->screen_rect.pos.x, y + this->screen_rect.pos.y},
            ...
            screen.SetPixel({x + this->screen_rect.pos.x, y + this->screen_rect.pos.y}, color);
        }
    }
}

/* Will draw a window using the level's drawbuffer: */
void TankView::Draw(ScreenRenderSurface & screen)
{
    ...
    for (int y = 0; y < this->screen_rect.size.y; y++) {
        for (int x = 0; x < this->screen_rect.size.x; x++)
        {
            ...
            screen.SetPixel({screen_x, screen_y}, color);
        }
    }

    /*
    ShapeRenderer::DrawCircle(screen->GetScreenSurface(), Position{this->screen_rect.Center()}, 35,
                              Palette.Get(Colors::Blank),
                                 Palette.Get(Colors::RadarOutline));
    */
...
void StatusBar::Draw(ScreenRenderSurface & screen)
{
    ...
    for (int y = 0; y < this->screen_rect.size.y; y++) {
        for (int x = 0; x < this->screen_rect.size.x; x++) {
            ...
            screen.SetPixel({x + this->screen_rect.pos.x, y + this->screen_rect.pos.y}, c);
        }
    }
}

void BitmapRender::Draw(ScreenRenderSurface & screen)
{
    this->data->Draw(screen, this->screen_rect.pos,
                     ImageRect{{0, 0}, {this->data->GetSize().x, this->data->GetSize().y}}, this->color);
}

void LivesLeft::Draw(ScreenRenderSurface & screen)
{
...
void Crosshair::Draw(ScreenRenderSurface & surface)
{
    if (!is_hidden)
        Parent::Draw(surface);
}

void ResourcesMinedDisplay::Draw(ScreenRenderSurface & screen)
{
    /* Draw outline and background */
    ShapeRenderer::DrawFilledRectangle(screen, Rect{this->screen_rect}, true,
