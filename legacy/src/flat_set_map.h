// Original implementation by Elnar Dakeshov 2017.
...
class flat_set : private std::vector<Key, Allocator>
{
  ...
  public:
    ...
    /*
     * Inserts the element into sorted set and shifts the elements via rotate
     */
    template <typename... Args>
    std::pair<iterator, bool> emplace(Args &&... args)
    {
        container_type::emplace_back(std::forward<Args>(args)...);
        auto lower = std::ranges::lower_bound(*this, container_type::back(), comp);
    ...
    /*
     * Elements are already sorted, look up via binary search 
     */
    ...
    iterator erase(const_iterator pos) { return container_type::erase(pos); }
    ...
    {
        assert(std::is_sorted(other.begin(), other.end(), comp));
        flat_set set;
  ...
  public:
    ...
    struct value_compare
    {
      ...
      public:
        ...
        bool operator()(const value_type & lhs, const value_type & rhs) const { return keyComp(lhs.first, rhs.first); }
    };
  ...
  public:
    ...
    iterator find(const key_type & key)
    {
        auto lower = std::lower_bound(
            begin(), end(), key, [&](const value_type & val, const key_type & key) { return keyComp(val.first, key); });
