...
DirectionF RandomAimDirection()
{
    ...
    return DirectionF::FromAbnormal(VectorF{static_cast<float>(x), static_cast<float>(y)});
}
} // namespace
...
ControllerOutput TwitchAI::Twitch(const PublicTankInfo & tank_info)
{
    ...
    if (!this->time_to_change)
    {
        ...
        {
            this->spd = Random.Bool(500) ? Speed{Random.Bool(500) ? 1 : -1, 0} : Speed{0, Random.Bool(500) ? 1 : -1};
        }
        this->shoot = Random.Bool(300);
        if (this->shoot)
        {
            this->aim_dir = RandomAimDirection();
        }
        else
        {
            this->aim_dir = DirectionF::FromAbnormal(SpeedF{this->spd});
        }
    }

    this->time_to_change--;
    return ControllerOutput{
        .move{.speed = SpeedF{this->spd}},
        .shoot{
            .aim_dir = this->aim_dir,
            .is_shooting_primary = this->shoot,
        }};
}
