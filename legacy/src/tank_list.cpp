...
Tank * TankList::GetTankAtPoint(Position query_pos, TankColor ignored)
{
    for (Tank & tank : *this)
    {
        ...
        if (sprites::TANK_SPRITE[tank.GetDirection().ToIntDirection()][pos.y][pos.x])
            return &tank;
    }
    return nullptr;
}
    ...
    for (Tank & otherTank : *this)
    {
        ...
        for (int ty = ly; ty <= uy; ty++)
            for (int tx = lx; tx <= ux; tx++)
                if (sprites::TANK_SPRITE[dir][ty - pos.y + 3][tx - pos.x + 3] &&
                    sprites::TANK_SPRITE[rotation][ty - position.y + 3][tx - position.x + 3])
                    return true;
    }
