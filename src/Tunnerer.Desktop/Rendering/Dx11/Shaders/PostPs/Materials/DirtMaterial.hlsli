// ============================================================================
// Materials/DirtMaterial.hlsli — Procedural dirt overlay
// ============================================================================
//
// Fully procedural dirt rendering that replaces the base terrain color with
// a rich, organic soil appearance. Unlike stone (which modulates the existing
// color), dirt generates its own palette from dry/wet/rusty soil tones and
// applies it via lerp at `dirtMask` strength.
//
// Layer stack:
//   1. Domain warping      — low-frequency warp fields break up grid patterns
//   2. Base relief         — broad bumpy terrain with normal-mapped diffuse
//   3. Macro landform      — hills, cliff bands, plateaus, terraces
//   4. Clump / moisture    — large organic patches of dry vs wet soil
//   5. Pores & grain       — medium-frequency detail
//   6. Micro detail        — high-frequency dust, spark, ridges (6 layers)
//   7. Fissures & cracks   — irregular fracture networks (3 scales)
//   8. Pebbles             — sparse mineral inclusions
//   9. Clod rims & sparkle — subtle edge highlights and micro-specular
// ============================================================================
...
void ApplyDirtMaterial(inout float3 color, float2 worldCell, float dirtMask, float3 lightDir)
{
    ...
    // ---- 1. Domain warp fields --------------------------------------------
    // Two warp layers at different frequencies distort the sampling domain
    // to prevent axis-aligned patterns.
    float2 warp0 = float2(
    ...
    // ---- 2. Base relief with normal mapping -------------------------------
    ...
    // ---- 7a. Irregular fissures (medium scale) ----------------------------
    // Warped directional ridges at multiple angles to avoid rectangular shapes.
    ...
    // ---- Color composition ------------------------------------------------
    // Build final dirt color from soil palette, modulated by all detail layers.
    ...
    // Apply fissures and macro features.
    ...
    // ---- 9. Clod rims and mineral sparkle ---------------------------------
