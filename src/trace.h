...
template <typename TimeUnit = std::chrono::microseconds, typename Clock = std::chrono::high_resolution_clock>
class Stopwatch
{
...
template <TraceLevel debug_level, typename... TPrintfArgs,
          typename std::enable_if_t<(debug_level <= DEBUG_TRACE_LEVEL), int> = 0>
...
template <int TraceLevel, typename TimeUnit = std::chrono::microseconds,
          typename Clock = std::chrono::high_resolution_clock, typename = void>
struct MeasureFunction
{
    MeasureFunction(std::string_view){};
    void Finish() {}
};

template <int TraceLevel, typename TimeUnit, typename Clock>
  ...
  public:
    ...
    void Finish()
    {
        if (!is_done)
        {
            ...
            DebugTrace<TraceLevel>("%s took %lld.%03lld ms \n", name.data(), watch.GetElapsed().count() / 1000,
                                   watch.GetElapsed().count() % 1000);
            is_done = true;
        }
    }
};
