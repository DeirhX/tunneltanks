...
static void flood_fill(Braid * b, int x, int y)
{
    ...
    if (x > 0 && !b->data[y * b->size.x + (x - 1)].right)
        flood_fill(b, x - 1, y);
    if (x < b->size.x - 1 && !b->data[y * b->size.x + x].right)
        flood_fill(b, x + 1, y);
    if (y > 0 && !b->data[y * b->size.x + x].up)
        flood_fill(b, x, y - 1);
    if (y < b->size.y - 1 && !b->data[(y + 1) * b->size.x + x].up)
        flood_fill(b, x, y + 1);
}
...
static int braid_makes_dead_end(Braid * b, int x, int y, Dir d)
{
    ...
    if (braid_node_dead_end(b, x, y, d))
        return 1;
...
static void braid_populate(Braid * b)
{
    ...
    for (i = 0; i < b->size.x * b->size.y * 2; i++)
    {
        ...
        if ((y == 0 && w[i].d == DIR_UP) || (x == b->size.x - 1 && w[i].d == DIR_RIGHT))
            continue;

        /* Check that this edge won't make a dead end bit: */
        if (braid_makes_dead_end(b, w[i].x, w[i].y, w[i].d))
            continue;

        /* Set this wall: */
        if (w[i].d == DIR_UP)
            b->data[w[i].y * b->size.x + w[i].x].up = 1;
        else if (w[i].d == DIR_RIGHT)
            b->data[w[i].y * b->size.x + w[i].x].right = 1;
        ...
        if (!braid_is_connected(b))
        {
            ...
            if (w[i].d == DIR_UP)
                b->data[w[i].y * b->size.x + w[i].x].up = 0;
            else if (w[i].d == DIR_RIGHT)
                b->data[w[i].y * b->size.x + w[i].x].right = 0;
        }
    }
